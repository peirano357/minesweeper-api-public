 ...
 */
app.controller('AppCtrl', ['$rootScope', '$scope', '$state', '$swipe', '$translate', '$localStorage', '$window', '$document', '$timeout', 'cfpLoadingBar', 'Fullscreen', 'gameSessionFactory',
    function ($rootScope, $scope, $state, $swipe, $translate, $localStorage, $window, $document, $timeout, cfpLoadingBar, Fullscreen, gameSessionFactory) {

        /* BEGIN - MANAGE SAVED GAMESMENU */
        $scope.reloadSaves = function () {
            $scope.savedGames = [];
            doReloadSaves();
        }

        function doReloadSaves() {

            var token = getCurrentToken();
            gameSessionFactory.getGameSessionListForUser(token).error(function (response) {

            }).success(function (response) {
                $scope.savedGames = response.data;
            });

            $scope.showGameSession = function (game) {
                $state.go('app.ui.gamedetail', {id: game[0]});
            }
        }

        doReloadSaves();
        /* END - MANAGE SAVED GAMESMENU */
