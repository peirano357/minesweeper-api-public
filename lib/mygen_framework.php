...
class BusinessObject {
    ...
    function set_data($field, $value) {
        $this->data[$field] = utf8_decode($value);
    }

    function set_data_clean($field, $value) {
    ...
    function sql_escape($field, $value) {
        ...
        if (array_key_exists($field, $this->field_metadata)) {
            ...
            if ($field == 'content') {
                $value = $data_objects[$this->db_key]->sql_noescape_value($datatype, $value);
            } else {
...
class BusinessObjectCollection {
    ...
    function get_totalcount_sql($where) {
        global $data_objects;
    ...
    function add_sort() {
        ...
        if (($arg_count == 2) || ($arg_count == 1)) {
            $sortAsc = true;
            if ($arg_count == 2)
                $sortAsc = func_get_arg(1);

            //TODO: need to validate fieldname
            $field = func_get_arg(0);
            $field = $data_objects[$this->db_key]->sql_escape_fieldname($field);
            $this->sort[$field] = ($sortAsc ? "DESC" : "ASC");
        }
        else {
            return false;
        }
